...
public class GameSparksFriendsManager : IFriendsManager
{
    ...
    {
        new GameSparks.Api.Requests.SocialLeaderboardDataRequest().SetDontErrorOnNotSocial(true).SetLeaderboardShortCode("LB.Level." + PlayerPrefs.GetInt("OpenLevel")).SetEntryCount(3).Send((response) =>
        {
            if (!response.HasErrors)
            {
